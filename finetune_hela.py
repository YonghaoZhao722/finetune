...
def get_dataloader(split, patch_shape, batch_size, train_instance_segmentation):
    ...
    loader = torch_em.default_segmentation_loader(
        ...
        sampler=MinInstanceSampler(),  # 确保采样包含实例的区域
        shuffle=True, raw_transform=sam_training.identity,
    )
    return loader
...
def run_training(checkpoint_name, model_type, train_instance_segmentation):
    ...
    device = torch.device("cuda") if torch.cuda.is_available() else torch.device("cpu") # the device used for training
